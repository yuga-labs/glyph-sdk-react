...
export const GlyphUserDataProvider = ({ children }: GlyphUserDataProviderProps) => {
    ...
    const { data: user, refetch: refetchUserQuery, error: userError } = useQuery<GlyphWidgetUser | null>({
    ...
    const refreshBalances = useCallback(
        async (force: boolean = false, cbs?: Record<string, (diffAmount: number) => void>) => {
            ...
            try {
                ...
                const res = await glyphApiFetch("/api/widget/balances");
                if (res.ok) {
        ...
        [authenticated, balances, balancesLastRefreshed, balancesLoading, ready, user, glyphApiFetch]
    );
    ...
    }, [ready, authenticated, glyphApiFetch, refreshBalances, balances]);
